...
class LFVILA_QA_Multichoice(nn.Module):
    def __init__(self, args, config):
        ...
        self.text_encoder = TextEncoderForMultichoice(args, config=bert_config)
         # Nếu không có checkpoint, load PhoBERT pretrained
        if config.WEIGHTS.bert_weight == '':
            print("🇻🇳 Loading PhoBERT pretrained weights...")
            from transformers import AutoModel
            phobert = AutoModel.from_pretrained('vinai/phobert-base')
            
            # Copy embeddings
            self.text_encoder.bert.embeddings.word_embeddings.weight.data.copy_(
                phobert.embeddings.word_embeddings.weight.data
            )
            self.text_encoder.bert.embeddings.position_embeddings.weight.data.copy_(
                phobert.embeddings.position_embeddings.weight.data
            )
            
            # Copy encoder layers
            for i in range(len(self.text_encoder.bert.encoder.layer)):
                if i < len(phobert.encoder.layer):
                    self.text_encoder.bert.encoder.layer[i].load_state_dict(
                        phobert.encoder.layer[i].state_dict()
                    )
            
            print("✅ PhoBERT weights loaded successfully!")
        ...
        self.vision_proj = nn.Linear(1024, 768) # Cầu nối Swin(1024) -> PhoBERT(768)
        # Khởi tạo weight ngẫu nhiên chuẩn để tránh bị loss NaN
    ...
    def forward(self, video_frames, text_ids, attention_mask=None, labels = None,span_labels=None, span_label_weights=None,):
        ...
        # print(f"👉 DEBUG VIDEO SHAPE: {video_stage1_embd.shape}")
        if hasattr(self, 'vision_proj'): 
             # Nếu 1024 ở cuối (Channel Last)
             if video_stage1_embd.shape[-1] == 1024:
                 video_stage1_embd = self.vision_proj(video_stage1_embd)
             # Nếu 1024 ở dimension 1 (Channel First)
             elif len(video_stage1_embd.shape) > 1 and video_stage1_embd.shape[1] == 1024:
                 video_stage1_embd = video_stage1_embd.permute(0, 2, 3, 4, 1)
                 video_stage1_embd = self.vision_proj(video_stage1_embd)
                 video_stage1_embd = video_stage1_embd.permute(0, 4, 1, 2, 3)
        # video_stage1_embd = self.video_token_pos(video_stage1_embd)
        # Lấy embedding gốc từ module
        s_pos = self.video_token_pos.s_pos_embed # [1, 1, 96, 768]
        t_pos = self.video_token_pos.t_pos_embed # [1, 8, 1, 768]
        
        # Kiểm tra kích thước hiện tại của video (ví dụ là 6)
        curr_spatial = video_stage1_embd.shape[2] # [B, T, Spatial, C]
        
        if s_pos.shape[2] != curr_spatial:
            # print(f"⚠️ Interpolating Pos Embed: {s_pos.shape[2]} -> {curr_spatial}")
            # Reshape để interpolate: [Batch, Channel, Length]
            # [1, 1, 96, C] -> Squeeze -> [96, C] -> Permute -> [1, C, 96]
            s_pos_t = s_pos.squeeze(0).squeeze(0).permute(1, 0).unsqueeze(0)
            
            # Nội suy tuyến tính về kích thước mới (ví dụ 6)
            s_pos_t = F.interpolate(s_pos_t, size=curr_spatial, mode='linear', align_corners=False)
            
            # Reshape lại về [1, 1, 6, C]
            s_pos = s_pos_t.squeeze(0).permute(1, 0).unsqueeze(0).unsqueeze(0)
            
        # Cộng thủ công (Manual Add)
        video_stage1_embd = video_stage1_embd + s_pos + t_pos
