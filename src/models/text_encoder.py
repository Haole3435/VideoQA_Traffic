...
class TextEncoderForMultichoice(BertPreTrainedModel):
    ...
    def forward(
        ...
        # Fix shape mismatch potential here by checking dimensions before view
        try:
            span_prediction = span_prediction.view(-1, num_choices, num_frame, 2)
            span_prediction = span_prediction.max(dim=1).values
        except Exception:
            # Fallback if dimensions don't match (e.g. batch size=1 vs choices)
            pass
        # span_prediction = span_prediction.view(-1, num_choices, num_frame, 2)
        # span_prediction = span_prediction.max(dim=1).values
        ...
        span_loss = torch.tensor(0.0).to(reshaped_logits.device)
        span_acc = torch.tensor(0.0).to(reshaped_logits.device)
        use_span_loss = getattr(self.config, 'use_span_loss', False)
        if use_span_loss and span_labels is not None:
            try:
                loss_fct = CrossEntropyLoss(reduction='none')
                
                # Ensure shapes match before calculation
                pred_flat = span_prediction.flatten(0,1)
                label_flat = span_labels.flatten(0,1)
                weight_flat = span_label_weights.flatten(0,1)
                
                if pred_flat.shape[0] == label_flat.shape[0]:
                    span_loss_calc = loss_fct(pred_flat, label_flat)
                    span_loss = (span_loss_calc * weight_flat).mean()
                    span_acc = (pred_flat.max(dim=-1)[1] == label_flat).float().mean(dim=0, keepdim=True)
                else:
                    # Skip calculation if shapes mismatch (e.g. 16 frames vs 8 labels)
                    pass 
            except Exception as e:
                # Silently ignore span loss errors to prevent training crash
                pass
            # loss_fct = CrossEntropyLoss(reduction='none')
