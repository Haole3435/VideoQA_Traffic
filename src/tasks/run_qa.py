import os
import sys
sys.path.append('/mnt/VideoQA_Traffic/LF-VILA')
...
def main():
    ...
    dataloader_train = list(dataloader_trains.values())[0]
    ...
    if config.qa_type == 'multichoice':
        ...
        # trainer = Trainer_QA_Classification(args, config, model_engine, optimizer, scheduler, dataloader_train, dataloader_vals['OCRDataset-val'])
        trainer = Trainer_QA_Multichoice(args, config, model_engine, optimizer, scheduler, dataloader_train, dataloader_vals['OCRDataset-val'])
    ...
    trainer.evaluate(dataloader_vals['OCRDataset-val'], stage=2)
