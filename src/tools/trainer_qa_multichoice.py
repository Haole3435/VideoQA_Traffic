...
class Trainer_QA_Multichoice():
    ...
    def evaluate(self, dataloader_val, stage=2):
        ...
        for step, batch in enumerate(dataloader_val):
            ...
            if batch.get('temporal_labels') is not None:
                span_labels = batch['temporal_labels'].to(self.local_rank)
            else:
                bs = labels.size(0)
                span_labels = torch.zeros((bs, 8), dtype=torch.long).to(self.local_rank)

            if batch.get('temporal_label_weights') is not None:
                span_label_weights = batch['temporal_label_weights'].to(self.local_rank)
            elif batch.get('span_label_weights') is not None:
                span_label_weights = batch['span_label_weights'].to(self.local_rank)
            else:
                bs = labels.size(0)
                span_label_weights = torch.ones((bs, 8), dtype=torch.float).to(self.local_rank)
            # span_labels = batch['temporal_labels'].to(self.local_rank)
            # span_label_weights = batch['temporal_label_weights'].to(self.local_rank)
